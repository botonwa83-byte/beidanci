...
console.log(
  `Added ${addedRootWords} new root-based words from ${newRoots.length} new roots`,
);
...
if (totalWords < 4000) {
  ...
  data.roots.forEach(r => (totalRootWords += r.words.length));
  totalSuppWords = 0;
  Object.values(data.supplement).forEach(
    list => (totalSuppWords += list.length),
  );
